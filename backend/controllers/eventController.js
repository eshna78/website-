...
const getEventHighlights = async (req, res) => {
  try {
    const events = await Event.find({
      status: 'Completed',
      highlights: { $exists: true, $ne: [] }
    })
    .select('title highlights date location type')
    .sort({ date: -1 })
    .limit(10);
...
const addEventHighlight = async (req, res) => {
  try {
    const event = await Event.findById(req.params.id);
    ...
    if (event.createdBy.toString() !== req.user.id && req.user.role !== 'admin') {
      return res.status(403).json({
        success: false,
        message: 'Not authorized to add highlights for this event'
      });
    }
    ...
    res.json({
      success: true,
      message: 'Event highlight added successfully',
      data: event
    });
  } catch (error) {
    res.status(400).json({
