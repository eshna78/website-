...
import { Calendar, MapPin, Clock, Users, Filter, Search, Plus, Star, Eye, ChevronRight, Image, Award, TrendingUp } from 'lucide-react';
...
const Events = () => {
  ...
  const [eventHighlights, setEventHighlights] = useState<Event[]>([]);
  ...
  const fetchEventHighlights = async () => {
    try {
      const response = await fetch('http://localhost:5000/api/events/highlights');
      const data = await response.json();
      
      if (data.success) {
        setEventHighlights(data.data);
      }
    } catch (error) {
  ...
  return (
    <div>
              ...
              <button
                ...
                className={`px-6 py-3 rounded-md font-bold transition-colors ${
                  activeTab === 'upcoming' 
                    ...
                    : 'bg-transparent border-2 border-yellow-400 text-yellow-400 hover:bg-yellow-400 hover:text-green-900'
                }`}
              >
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="grid grid-cols-2 md:grid-cols-4 gap-6 text-center">
            <div className="p-6">
              ...
              <p className="text-3xl font-bold text-green-800">{events.length}</p>
            ...
            <div className="p-6">
              ...
              <p className="text-3xl font-bold text-green-800">{upcomingEvents.length}</p>
            ...
            <div className="p-6">
              ...
              <p className="text-3xl font-bold text-green-800">{pastEvents.length}</p>
      ...
      {/* Event Countdown Section - Only for upcoming tab */}
      {activeTab === 'upcoming' && upcomingEvents.length > 0 && (
        <section className="bg-white py-12">
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <h2 className="text-2xl font-bold text-green-800 mb-8 text-center">Next Events</h2>
                ...
                {eventHighlights.map(event => (
                  <div key={event._id} className="bg-white rounded-lg shadow-md overflow-hidden hover:shadow-lg transition-shadow">
                    ...
                    <div className="p-6">
                      ...
                      {event.highlights && event.highlights.length > 0 && (
                        <div className="space-y-3">
                          {event.highlights.slice(0, 2).map((highlight, index) => (
                            <div key={index} className="border-l-4 border-green-800 pl-3">
                      ...
                      <button 
                        onClick={() => handleEventClick(event)}
                        className="mt-4 flex items-center text-green-800 hover:text-green-700 font-semibold"
                      >
                        View Full Gallery
      ...
      {/* Search and Filter - Only for upcoming and past tabs */}
      ...
      {/* Events Grid - Only for upcoming and past tabs */}
      {(activeTab === 'upcoming' || activeTab === 'past') && (
        <section className="py-16 bg-white">
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
